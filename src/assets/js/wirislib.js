...
function saveOrbiitQuestion() {
  ...
  var n = $("input[name^='is_correct']").length;
  var arrayelecorrect = $("input[name^='is_correct']");
  var checkedArr = [];
  var answersdata = [];

  arrayelecorrect.each(function (index) {
    checkedArr.push($(this).is(":checked"));
  });
  //console.log(checkedArr);


  var answer1 = getAnswer1Data();
  var answer2 = getAnswer2Data();
  var answer3 = getAnswer3Data();
  var answer4 = getAnswer4Data();


  answersdata.push({
    "answer": answer1,
    "is_correct": checkedArr[0]
  });
  answersdata.push({
    "answer": answer2,
    "is_correct": checkedArr[1]
  });
  answersdata.push({
    "answer": answer3,
    "is_correct": checkedArr[2]
  });
  answersdata.push({
    "answer": answer4,
    "is_correct": checkedArr[3]
  });

  //Make answers data

  //     var n = $("textarea[name^='answers']").length;
  //     var arrayele = $("textarea[name^='answers']");
  //    arrayele.each(function( index ) {
  //      answersdata.push({"answer":$( this ).val(),"is_correct" : checkedArr[index]});
  //   });
  ...
  var opt = $("#opt").val();
  var is_hidden = $("#is_hidden").is(":checked");
  ...
  $.post("ajax/savedata.php", {
    ...
    if (result.status == '200') {
      ...
      self.location = "add_question.php?tagid=" + tag_id + "&concept_id=" + concept_id;
    } else {
...
String.prototype.getMatchIndices = function (find) {
  var indices = [],
    data, exp = (typeof find == 'string' ? new RegExp(find, 'g') : find);
...
function createEditorInstance(lang, wiriseditorparameters) {
  ...
  CKEDITOR.on("instanceReady", function (event) {
    ...
    //CKEDITOR.instances.solution.focus();
    updateFunction();
  });
}
...
if (typeof urlParams !== 'undefined') {
  if (urlParams[0] === 'language' && urlParams[1].length >= 2) {
    createEditorInstance(urlParams[1], {});
  } else {
...
function getSolutionData() {
  return CKEDITOR.instances.solution.getData();
}


function getAnswer1Data() {
  return CKEDITOR.instances.answer1.getData();
}

function getAnswer2Data() {
  return CKEDITOR.instances.answer2.getData();
}

function getAnswer3Data() {
  return CKEDITOR.instances.answer3.getData();
}

function getAnswer4Data() {
  return CKEDITOR.instances.answer4.getData();
}
...
function resetEditor(lang, wiriseditorparameters) {
  ...
  CKEDITOR.instances.solution.destroy();
